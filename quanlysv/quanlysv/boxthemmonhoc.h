...
#pragma endregion
	private: System::Void save_Click(System::Object^ sender, System::EventArgs^ e) {
		String^ newSubject = Addsub->Text;
		if (String::IsNullOrWhiteSpace(newSubject)) {
			MessageBox::Show(L"Vui lòng nhập tên môn học mới!");
			return;
		}
		// Ghi môn học mới vào file subjects.txt
		System::IO::StreamWriter^ sw = gcnew System::IO::StreamWriter("subjects.txt", true);
		sw->WriteLine(newSubject);
		sw->Close();
		MessageBox::Show(L"Thêm môn học thành công!");
		// Đóng form và quay lại giao diện trang chủ
		this->Close();
	}
